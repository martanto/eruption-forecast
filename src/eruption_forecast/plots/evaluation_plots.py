...
# ---------------------------------------------------------------------------
# Aggregate (multi-seed) metric computation functions (no matplotlib)
# ---------------------------------------------------------------------------


def compute_aggregate_roc_data(
    y_trues: np.ndarray | list[np.ndarray],
    y_probas: list[np.ndarray],
) -> pd.DataFrame:
    """Compute aggregate ROC curve statistics across multiple seeds.

    Interpolates each seed's ROC curve onto a shared 200-point FPR grid and
    returns the mean and standard deviation of the true positive rate at each
    grid point. No figure is rendered.
    ...
    Returns:
        pd.DataFrame: DataFrame with columns ``fpr``, ``mean_tpr``, ``std_tpr``
            and 200 rows (one per FPR grid point). The structure matches the
            data CSV saved by ``plot_aggregate_roc_curve()``.

    Examples:
        >>> data = compute_aggregate_roc_data(y_true, y_probas_list)
        >>> data.to_csv("roc_data.csv", index=False)
        >>> # mean AUC via trapezoidal integration
        >>> import numpy as np
        >>> auc_approx = np.trapz(data["mean_tpr"], data["fpr"])
    """
    ...
    return pd.DataFrame({"fpr": fpr_grid, "mean_tpr": mean_tpr, "std_tpr": std_tpr})


def compute_aggregate_pr_data(
    y_trues: np.ndarray | list[np.ndarray],
    y_probas: list[np.ndarray],
) -> pd.DataFrame:
    """Compute aggregate Precision-Recall curve statistics across multiple seeds.

    Interpolates each seed's PR curve onto a shared 200-point recall grid and
    returns the mean and standard deviation of precision at each grid point. No
    figure is rendered.
    ...
    Returns:
        pd.DataFrame: DataFrame with columns ``recall``, ``mean_precision``,
            ``std_precision`` and 200 rows. The structure matches the data CSV
            saved by ``plot_aggregate_precision_recall_curve()``.

    Examples:
        >>> data = compute_aggregate_pr_data(y_true, y_probas_list)
...
def compute_aggregate_calibration_data(
    y_trues: np.ndarray | list[np.ndarray],
    y_probas: list[np.ndarray],
    n_bins: int = 10,
) -> pd.DataFrame:
    """Compute aggregate calibration curve statistics across multiple seeds.

    Computes each seed's calibration curve on a uniform ``n_bins``-point
    probability grid, then returns the mean and standard deviation of the
    fraction of positives at each bin. No figure is rendered.
    ...
    Returns:
        pd.DataFrame: DataFrame with columns ``prob_bin``,
            ``mean_frac_positives``, ``std_frac_positives`` and ``n_bins``
            rows. The structure matches the data CSV saved by
            ``plot_aggregate_calibration()``.

    Examples:
        >>> data = compute_aggregate_calibration_data(y_true, y_probas_list)
...
def compute_aggregate_prediction_distribution_data(
    y_trues: np.ndarray | list[np.ndarray],
    y_probas: list[np.ndarray],
) -> pd.DataFrame:
    """Pool predicted probabilities from all seeds by true class.

    Concatenates the predicted probabilities from every seed into a single
    flat array, retaining the associated true label for each sample. No
    figure is rendered.
    ...
    Returns:
        pd.DataFrame: DataFrame with columns ``y_proba`` and ``y_true``
            containing the pooled probabilities and integer labels from all
            seeds. The structure matches the data CSV saved by
            ``plot_aggregate_prediction_distribution()``.

    Examples:
        >>> data = compute_aggregate_prediction_distribution_data(y_true, y_probas_list)
        >>> data.to_csv("pred_dist_data.csv", index=False)
        >>> # Separate by class
        >>> erupted = data[data["y_true"] == 1]["y_proba"]
    """
    if isinstance(y_trues, np.ndarray):
        y_trues = [y_trues] * len(y_probas)
    ...
    return pd.DataFrame({"y_proba": pooled_probas, "y_true": pooled_trues.astype(int)})


def compute_aggregate_confusion_matrix_data(
    y_trues: np.ndarray | list[np.ndarray],
    y_preds: list[np.ndarray],
) -> pd.DataFrame:
    """Accumulate a summed confusion matrix across all seeds.

    Adds raw confusion matrices from every seed into a single 2×2 matrix.
    ...
    Returns:
        pd.DataFrame: 2×2 DataFrame of raw (un-normalised) summed counts with
            index and columns ``["not_erupted", "erupted"]``. The structure
            matches the data CSV saved by ``plot_aggregate_confusion_matrix()``.

    Examples:
        >>> data = compute_aggregate_confusion_matrix_data(y_true, y_preds_list)
        >>> data.to_csv("confusion_matrix_data.csv")
        >>> tn = data.loc["not_erupted", "not_erupted"]
        >>> tp = data.loc["erupted", "erupted"]
    """
    if isinstance(y_trues, np.ndarray):
...
def compute_aggregate_threshold_data(
    y_trues: np.ndarray | list[np.ndarray],
    y_probas: list[np.ndarray],
) -> pd.DataFrame:
    """Compute aggregate threshold-vs-metric statistics across multiple seeds.

    Sweeps 101 thresholds from 0 to 1, computes precision, recall, F1, and
    balanced accuracy for each seed at each threshold, then returns the mean
    and standard deviation across seeds. No figure is rendered.
    ...
    Returns:
        pd.DataFrame: DataFrame with 101 rows and columns ``threshold``,
            ``mean_precision``, ``std_precision``, ``mean_recall``,
            ``std_recall``, ``mean_f1``, ``std_f1``,
            ``mean_balanced_accuracy``, ``std_balanced_accuracy``. The
            structure matches the data CSV saved by
            ``plot_aggregate_threshold_analysis()``.

    Examples:
        >>> data = compute_aggregate_threshold_data(y_true, y_probas_list)
        >>> data.to_csv("threshold_data.csv", index=False)
        >>> # Find threshold that maximises mean F1
        >>> best_thresh = data.loc[data["mean_f1"].idxmax(), "threshold"]
    """
    thresholds = np.linspace(0, 1, 101)
...
def compute_aggregate_feature_importance_data(
    models: list[BaseEstimator],
    feature_names: list[str],
) -> pd.DataFrame | None:
    """Compute mean and std feature importances across multiple seeds.

    Collects ``feature_importances_`` from each seed's model, stacks them,
    and returns the mean and standard deviation for every feature sorted by
    descending mean importance. Returns None when no model exposes feature
    importances. No figure is rendered.
    ...
    Returns:
        pd.DataFrame | None: DataFrame with columns ``feature``,
            ``mean_importance``, ``std_importance`` sorted by descending mean
            importance (all features included, not just top-N). Returns None
            when no model exposes ``feature_importances_``. The structure
            matches the data CSV saved by
            ``plot_aggregate_feature_importance()``.

    Examples:
        >>> data = compute_aggregate_feature_importance_data(models_list, feature_names)
        >>> if data is not None:
    ...
    stacked = np.stack(all_importances, axis=0)
