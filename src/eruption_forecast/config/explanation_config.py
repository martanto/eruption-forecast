...
@dataclass
class ExplanationConfig(BaseConfig):
    """Configuration for ``ExplanationModel.__init__`` parameters.

    Captures the serializable arguments accepted by
    ``ExplanationModel.__init__``. The upstream ``model`` parameter is
    ...
    Attributes:
        ...
        prefix_config (str | None): Discriminator slugified into the
            ...
            ``"scenario 1"`` → ``explanation.scenario-1.config.yaml``).
    ...
    def to_dict(self) -> dict[str, Any]:
        """Convert the explanation configuration to a plain dictionary.
    ...
    def save(self, path: str, fmt: Literal["yaml", "json"] = "yaml") -> str:
        """Save the explanation configuration to *path*.
        ...
        if fmt == "json":
            ...
            with open(path, "w", encoding="utf-8") as f:
                f.write("# eruption-forecast ExplanationModel configuration\n")
                yaml.safe_dump(
                    data,
                    f,
    ...
    def load(cls, path: str) -> Self:
        """Load an explanation configuration from *path*.
        ...
        Returns:
            ExplanationConfig: A fully populated config instance.
