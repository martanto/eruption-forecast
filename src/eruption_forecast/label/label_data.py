...
class LabelData:
    def __init__(self, label_csv: str):
        ...
        # Example basename: label_2020-01-01_2020-12-31_ws-1_wo-0.5_sr-100_dtf-1.csv
        # ws-1 -> window_size = 1 (days)
        # wo-0.5 -> window_overlap = 0.5 (fraction/percentage)
        ...
        self.window_overlap = float(window_overlap.split("-")[0])
    ...
    def validate(self) -> None:
        ...
        Example: label_2020-01-01_2020-12-31_ws-1_wo-0.5_sr-100_dtf-1.csv
        """
        example_label_filename = (
            "label_2020-01-01_2020-12-31_ws-1_wo-0.5_sr-100_dtf-1.csv"
        )
        ...
        assert len(parts) == 7, (
            f"Label filename is invalid. Expected format: label_YYYY-MM-DD_YYYY-MM-DD_ws-X_wo-X_sr-X_dtf-X.csv. "
        ...
        assert window_size.split("-")[
            1
        ].isdigit(), f"Window size should be integer in days. Expected format: ws-X. Got: ws-{window_size}"

        # Asserting window overlap
        assert window_overlap.startswith(
            "wo-"
        ), f"Window overlap is invalid. Expected format: wo-X. Got: wo-{window_overlap}"
        assert window_overlap.split("-")[
            1
        ].isdigit(), f"Window overlap should be float in percentage. Expected format: wo-X. Got: wo-{window_overlap}"
        ...
        assert sampling_rate.split("-")[
            1
        ].isdigit(), f"Sampling rate should be float in Hz. Expected format: sr-X. Got: sr-{sampling_rate}"
        ...
        assert day_to_forecast.split("-")[
            1
        ].isdigit(), f"Day to forecast should be integer in days. Expected format: dtf-X. Got: dtf-{day_to_forecast}"
