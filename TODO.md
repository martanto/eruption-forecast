...
- [x] **BaseEnsemble + serialization refactor (2026-03-01)** — Extracted shared `save()`/`load()` logic into `BaseEnsemble` mixin; `SeedEnsemble` and `ClassifierEnsemble` inherit from it. Eliminated duplicated persistence boilerplate.
- [x] **Multi-metric learning curve support (2026-03-01)** — `ModelTrainer._compute_all_learning_curves()` now iterates over all `LEARNING_CURVE_SCORINGS` and produces a dict-keyed JSON per seed. Added `plot_learning_curve_grid()` to render one subplot per scoring metric. `ModelEvaluator.plot_learning_curve()` and `MultiModelEvaluator.plot_learning_curve()` updated accordingly.
- [x] **Apply `ensure_dir` consistently; remove UserWarning for tight_layout (2026-03-01)** — All `os.makedirs` calls replaced with `ensure_dir()` from `utils.pathutils`. Removed the `UserWarning` filter for `tight_layout` after upgrading to matplotlib where the warning no longer occurs.
- [x] **Cached file properties in BaseDataContainer (2026-03-01)** — Added `@cached_property` attributes to `BaseDataContainer` so filename-derived values are computed once and reused.
- [x] **Remove filename/basename/filetype properties (2026-03-01)** — Deleted the now-redundant `filename`, `basename`, and `filetype` properties from `BaseDataContainer`; callers use the `path` attribute directly.
- [x] **Add DEBUG setting and comments to `.env.example` (2026-03-01)** — Added `DEBUG=false` entry with inline comment to the example env file.
- [x] **Threshold metrics + model merging refactor (2026-03-01)** — Consolidated threshold-based metric helpers; unified `merge_models()` / `merge_classifier_models()` paths.
- [x] **Add learning curve plots — single-seed and multi-seed stability (2026-03-01)** — `ModelEvaluator.plot_learning_curve()` added; `MultiModelEvaluator.plot_learning_curve()` renders aggregate learning curves across seeds with ±1 std bands.
- [x] **Always save test datasets to disk (2026-03-01)** — `ModelTrainer.train_and_evaluate()` now unconditionally writes `X_test` and `y_test` CSVs to `tests/` alongside each seed model.
