...
- [x] **Refactor output directory structure + docstring fixes (2026-02-15)** - Restructured `ModelTrainer` output into `model-with-evaluation/` and `model-only/` subdirectories organised by `{classifier-slug}/{cv-slug}/`. Added `get_classifier_properties()` and `update_directories()` methods. Added `with_evaluation` and `grid_params` params to `ForecastModel.train()`. Fixed `get_metrics()` parameter rename (`classifier` → `classifier_model`). Added `slugify_class_name()` to `utils.py`. Fixed docstring typos (`pd.DataFRame`, `SLugify`, `Trainig`). Updated README output structure, path examples, and ForecastModel description.
- [x] **Code Review & Architecture Refactoring (2026-02-17)** - Fixed 4 type errors in `model_evaluator.py` (Series→ndarray conversions). Created `config/constants.py` module with 8 centralized constants (TRAIN_TEST_SPLIT, DEFAULT_CV_SPLITS, etc.). Extracted `ModelEvaluator._save_plot()` helper (eliminated 7x duplicate code, ~100 lines). Created `MetricsComputer` class and refactored `ModelEvaluator` to use composition pattern. Added `build_model_directories()` to `pathutils.py`. Replaced 12+ hardcoded values across codebase. All imports moved to top of files (PEP 8). Zero breaking changes (100% backward compatible). Type checker: ✅ 0 errors, Linter: ✅ 0 errors.
