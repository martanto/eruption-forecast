...
# %%
def integrate(data: np.ndarray, sampling_rate: float = 100.0, anchor_start_sample: Optional[int] = None) -> np.ndarray:
    displacements: np.ndarray = cumulative_trapezoid(data, dx=1./sampling_rate, initial=0)

    if verbose:
        print(f"displacements[{anchor_start_sample}]: {displacements[anchor_start_sample]}")
    ...
    if anchor_start_sample is not None:
        ...
        if verbose:
            print(f"after displacements[{anchor_start_sample}]: {displacements[anchor_start_sample]}")
...
def wrapped_indices(outlier_index, asymmetric_factor, subdomain_range, total_windows):
    ...
    start_index = int(outlier_index - asymmetric_factor_value) # Compute the index of the domain where the subdomain centered on the peak begins
    ...
    if end_index >= total_windows: # If end index exceeds data range
        index = list(range(end_index - total_windows)) # Wrap domain so continues from beginning of data range
        ...
        end = list(range(total_windows+start_index, total_windows)) # Wrap domains so continues at end of data range
        index.extend(end)
    else:
    ...
    return index
# %%
def get_data_for_day(day_index, utc_start_datetime: UTCDateTime, _station, extends: bool = False):
    # t0 = utc_datetime
    ...
    if verbose:
        print(f"day_index, start_sample, end_sample: {day_index}, ({start_sample} - {end_sample})")
    ...
    if verbose:
        print(f"ten_minutes_samples, total_windows: {ten_minutes_samples}, {total_windows}")

    # integrating
    displacement = integrate(data, sampling_rate=sampling_rate, anchor_start_sample=start_sample)
    ...
    for freq_min, freq_max in freq_bands:
        _all_data = abs(bandpass(data, freq_min, freq_max, sampling_rate)[start_sample:end_sample])*1.e9
        _all_displacement = abs(bandpass(displacement, freq_min, freq_max, sampling_rate)[start_sample:end_sample])*1.e9
    ...
    for window_index in range(total_windows):
        outlier, outlier_index, _ = detect_outliers(all_data[2][window_index * ten_minutes_samples:(window_index + 1) * ten_minutes_samples])
    ...
    for _all_data, band_name in zip(all_data, band_names):
        ...
        for window_index, outlier, outlier_index in zip(range(total_windows), outliers, outlier_indices):
            _rsam = _all_data[window_index * ten_minutes_samples:(window_index + 1) * ten_minutes_samples]
            rsam.append(np.mean(_rsam))

            if outlier:
                _outlier_index = wrapped_indices(outlier_index, asymmetric_factor, subdomain_range, total_windows)
    ...
    for ratio_index, ratio_name in enumerate(ratio_names):
        ...
        for window_index, outlier, outlier_index in zip(range(total_windows), outliers, outlier_indices):
            first_domain = all_displacement[ratio_index][window_index * ten_minutes_samples:(window_index + 1) * ten_minutes_samples]
            second_domain = all_displacement[ratio_index+1][window_index * ten_minutes_samples:(window_index + 1) * ten_minutes_samples]
