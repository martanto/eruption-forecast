    ...
    def update(self, ti=None, tf=None, n_jobs = None):
        ...
        for i in range(ndays):
            ...
            if not os.path.isfile(fl): 
                continue
            dfs.append(load_dataframe(fl, index_col=0, parse_dates=True, infer_datetime_format=True))
        # shutil.rmtree('_tmp')

        if len(dfs) == 0:
            raise ValueError(
                f"❌ update:: Cannot get data from temporary dir ('_tmp')"
            )
...
#             print(f"Next day : {_calculate_date.strftime('%Y-%m-%d')}")
    ...
    try:
        ...
        i0=(
                int(
                    (
                        t0 + i * _DAY - st.traces[0].meta['starttime']
                    )*F
                )
            +1
        )
        ...
        maxIdxs = []
        for k in range(m):
            outlier, maxIdx = outlierDetection(_datas[2][k*N:(k+1)*N]) # _datas[2][0:60000]
